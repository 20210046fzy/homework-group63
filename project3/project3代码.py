...
def length_extension_attack(data,h,x):
    m = Fill(str(data))
    p=Fill(str(int(m,2))+str(x))
    p=p[len(m):]
    n=int(len(p)/512)
    f=[]
    b=h
    for i in range(n):
        f.append(p[512 * i:512 * (i + 1)])
        w = Expand(f[i])
        b= Compress(w, b)
    return (m,str(m)+str(bin(x)[2:]),b)
if __name__ == "__main__":
    x = random.randint(1,2**18)
    data=random.randint(1,2**18)
    h1=SM3(str(data))
    (a,b,c)=length_extension_attack(data,h1,x)
    d=SM3(str(int(b,2)))
    print('填充消息:',a)
    print('伪造消息:',b)
    print('伪造hash值:',c)
    print('伪造消息的直接hash值:',d)
